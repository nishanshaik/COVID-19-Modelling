...
loadInterventions = function(p_workopen)
{
  list(
    ...
    base =list(home = diag(1,16,16),
               ...
               school = diag(1,16,16),
               others = diag(1,16,16)),
    # Region's lockdown--assume from XX March to XX June ## (Read Wuhan as the region under assessment) ##
                                    ...
                                    isolation_efficiency = 1) # The percentage of symptomatic cases that the health system can identify and isolate. Affects force of infection. 
{
  ...
  E[1,] = 0 
  Ic[1,] =  pInfected*sum(N_age)*0.31/16 #rpois(length(N_age),lambda = pInfected*sum(N_age)/16)  # 100 # Assign 100 infected person in each age group (TODO RELAX?)
  Isc[1,] = pInfected*sum(N_age)*(1-0.31)/16 
  R[1,] = 0 
...
scenario_6 = vector('list',nsim)      # Define Scenario_6: A lockdown with 33% workforce participation and 100% compliance, and nil isolation efficiency starting 1 July 2020 for two months. 
...
for(sim in 1:nsim)
{
  scenario_6[[sim]] = simulateOutbreakSEIcIscR(R0t = R0est[sim],
                                               dateStart = as.Date("2020-03-02"),
                                               dateStartIntenseIntervention = as.Date('2020-07-01'), 
                                               dateEndIntenseIntervention = as.Date('2020-08-31'),
                                               pWorkOpen = c(0.33, 0.50, 0.67, 1),
                                               numWeekStagger = c(3, 6, 9),
                                               pInfected = initialI,
                                               durInf = durInfSim,
