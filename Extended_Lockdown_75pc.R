...
loadInterventions = function(p_workopen)
{
  list(
    ...
    wuhanlockdown = list(home = diag(1,16,16),
                         work = diag(0.25,16,16),
                         school = diag(0,16,16),
                         others = diag(c(rep(0.25,4),rep(0.25,12)))),
    ...
    schcloseonly = list(home = diag(c(rep(1,4),rep(1,12))),
                        work = diag(1,16,16),
                        school = diag(0,16,16),
                        others = diag(c(rep(0.25,4),rep(1,12)))), 
...
scenario_3 = vector('list',nsim)      # Define Scenario_3: An extended double duration lockdown of 42 days with 75% lockdown efficiency with the population compliant as before.
...
for(sim in 1:nsim)
{
  scenario_3[[sim]] = simulateOutbreakSEIcIscR(R0t = R0est[sim],
                                               dateStart = as.Date("2020-03-02"),
                                               dateStartIntenseIntervention = as.Date('2020-03-25'), 
                                               dateEndIntenseIntervention = as.Date('2020-06-13'),
                                               pWorkOpen = c(0.33, .55, .77, 1),
                                               numWeekStagger = c(3, 6, 9),
                                               pInfected = initialI,
                                               durInf = durInfSim,
